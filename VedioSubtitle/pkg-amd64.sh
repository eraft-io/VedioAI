...
# 视频字幕翻译器 macOS AMD64 (Intel) 打包脚本
# 支持生成 PKG 和 DMG 两种格式
...
# 默认生成 DMG，可通过参数指定: ./pkg-amd64.sh pkg
BUILD_TYPE="${1:-dmg}"
...
rm -f build/pkg/VideoSubtitle-*-amd64.dmg
...
if [ -d "build/bin/VideoSubtitle.app" ]; then
    ...
    if [ "$BUILD_TYPE" = "pkg" ]; then
        ...
        echo ""
        echo "输出文件:"
        echo "  PKG: build/pkg/VideoSubtitle-${VERSION}-amd64.pkg"
        echo ""
        ...
        # 创建 DMG（拖动安装方式）
        echo "创建 AMD64 DMG 安装包（拖动安装）..."
        
        DMG_FILE="build/pkg/VideoSubtitle-${VERSION}-amd64.dmg"
        ...
        # 复制应用到临时目录
        cp -R "build/darwin/amd64/VideoSubtitle.app" "$DMG_TEMP_DIR/"
